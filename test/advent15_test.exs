defmodule Advent15Test do
  ...
  describe "generate next number for spoken numbers memory" do

    test "zero for never met numbers" do
      assert Advent15.next_number_for(%{last_spoken: 9, current_turn: 1, already_spoken: %{}}) == 0
      assert Advent15.next_number_for(%{last_spoken: 6, current_turn: 3, already_spoken: %{0 => 1, 3 => 2}}) == 0
    end

    test "already met numbers: the difference between current turn and position where most recently met" do
      assert Advent15.next_number_for(%{
        last_spoken: 0,
        current_turn: 4,
